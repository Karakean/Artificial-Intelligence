...
def train(model, train_data, valid_data=None, exp_dir='exp_00'):
    ...
    # my callbacks
    es_cbk = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=5)
    ckp_cbk = tf.keras.callbacks.ModelCheckpoint(exp_dir+'/model_best_weights', monitor='val_loss',
...
def get_random_hp(constants):
    ...
    dim_len = random.randint(hidden_dims_len_range[0], hidden_dims_len_range[1])
    hidden_dims=[]
    ...
    learning_rate = random.randint(lr_exponent_range[0], lr_exponent_range[1])
