...
def build_model(hp: FFN_Hyperparams):
    ...
    for i, dim in enumerate(hp.hidden_dims):
        if i == 0:
            model.add(layers.Dense(dim, activation=hp.activation_fcn, input_shape=[hp.num_inputs], name='ukryta_' + str(i)))
        else:
            model.add(layers.Dense(dim, activation=hp.activation_fcn, name='ukryta_'+str(i)))
